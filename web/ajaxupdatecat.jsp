...
<%
        ...
        String catid="", catname="", catimage="", path="";
       ...
       for(FileItem myitem:items)
       {
            if (myitem.isFormField())
            {
                ...
                if(itemName1.equals("catid")) //control's name - textbox name
               {
                ...
                else if(itemName1.equals("catname")) //control's name - textbox name
               {
                    catname=value;
                }
            }
            else
            {
                    ...
                    if(imagesize==0)
                    {
                        catimage=session.getAttribute("catimage").toString();
                    }
                            ...
                            || type.equals("image/gif")) && imagesize<400)
                        {
                            ...
                            String oldimage=session.getAttribute("catimage").toString();
                            ...
                            path=config.getServletContext().getRealPath("/") + "images\\" + catimage;
                ...
                if(successful==true)
                {
                        try
                            ...
                            try
                            {
                                String query="update categorytable set catname=?, catimage=? where catid=?";
                                PreparedStatement mystatement=myconnection.prepareStatement(query);
                                mystatement.setString(1, catname);
                                mystatement.setString(2, catimage);
                                mystatement.setString(3, catid);
                                
                                if(mystatement.executeUpdate()>0)
                                {
                                    out.print("<br>");
                                        out.print("<div class='alert alert-success' role='alert'><center><strong>"
                                                +"You have successfully updated Category!</strong></center></div>");
                                }
